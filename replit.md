...
- Phase 15: SaaS offering — subscriptions table (unique user_id) with Free/Pro($49)/Enterprise($249) plans and PLAN_LIMITS; Stripe billing via Replit connector (stripe-replit-sync, managed webhook at /api/stripe/webhook registered BEFORE express.json, products seeded); billing routes (/api/billing/subscription, /checkout, /portal); atomic quota enforcement inside the attestation transaction (402 on limit, conditional UPDATE guard); API key limit enforcement; hash-chain race fixed with pg_advisory_xact_lock + raw SQL transaction (verified: 15-way concurrent attests, chain fully valid); landing page pricing + "Why Hosted ForgeProof?" sections; dashboard Billing tab with usage meters, upgrade checkout, and Stripe portal
- Phase 14: Signature verification fix — added `signedAt` varchar column to store exact signing timestamp, fixing timestamp precision mismatch that caused all signatures to show as invalid. Updated seed data with GPT-5/OpenAI attestations alongside Claude/Anthropic. All domain references updated to forgeproof.flyingcloudtech.com
